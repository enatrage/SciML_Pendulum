...
class NIF_Paper(nn.Module):
    
    # Here, we define another custom version of the Paper NIF where we do not initialize the entirety of the shape network but only a part of it
    
    def __init__(self, cfg_shape_net, cfg_param_net):
        super(NIF_Paper, self).__init__()
        ...
        # Add input layer, connects the generated layer
        self.p_layers = [nn.Linear(in_features=self.param_generated_unit, out_features=self.param_hidden_units[0]), self.param_activation]
    ...
    def _call_shape_network(self, shape_input, latent_space):
        ...
        generated_W = latent_space[:, self.param_generated_unit * self.shape_i_dim].view(self.param_generated_unit, self.shape_i_dim)
        generated_b = latent_space[self.param_generated_unit * self.shape_i_dim]

        out = nn.Linear(shape_input, weight= generated_W, bias=generated_b)
...
class NIF_PaperSiren(nn.Module):
...
class NIF_PaperFourier(nn.Module):
    """
    This is a custom NIF model that uses Fourier features for the shape network.
    It is based on the paper "Fourier Features Let Networks Learn High Frequency Functions in Low Dimensional Domains" by Tancik et al.
    """
